...
假设有一个分布$p(x|\lambda)$，由参数$\lambda$控制，也许能找到一个合适的先验分布$p(\lambda)=\text{const}$. 如果$\lambda$是一个有K个状态的离散变量，则先验概率分散到每个状态的概率为$1/K$. 而对于连续变量，则有两个难题；第一个是如果$\lambda$无界，则该先验分布无法正确归一化。这种先验称为improper.第二个是当变量进行非线性变换的时候概率密度的变化。假设一个函数$h(\lambda)$是常数，如果将变量变成$\lambda=\eta^2$，那么$\hat{h}(\eta)=h(\eta^2)$同样也是常数。但是如果令密度函数$p_{\lambda}(\lambda)$是常数，则$\eta$的密度函数为：
$$
